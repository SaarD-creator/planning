...
# -----------------------------
# HULPFUNCTIE: fusie actief?
# -----------------------------
def fusie_actief(uur, fusienaam):
    return bool(assigned_map.get((uur, fusienaam)))
...
for attr in attracties_te_plannen:
    ...
    for vis_idx, vis_attr in enumerate(visuele_attrs):
        ws_out.cell(rij_out, 1, vis_attr).font = Font(bold=True)
    ...
    # cellen mergen per uur indien fusie actief
    if attr in fusienaam_naar_groep:
        for col_idx, uur in enumerate(sorted(open_uren), start=2):
            if fusie_actief(uur, attr):
                ws_out.merge_cells(
...
for idx, naam in enumerate(extras_flat, start=1):
    ...
    for col_idx, uur in enumerate(sorted(open_uren), start=2):
        ...
        cell = ws_out.cell(rij_out, col_idx, cell_naam)
        cell.alignment = center_align
        cell.border = thin_border
        if cell_naam:
            cell.fill = PatternFill(
